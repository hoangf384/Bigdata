...
spark = (
    SparkSession.builder.appName("ETL-30Days")
    .master(SPARK_MASTER)
...
save_path = "/data/destination_2/log_content_30_days/"
# -------------------------
# Helpers: IO
# -------------------------
...
def save_data(result, path):
    ...
    """
    (result.repartition(5).write.mode("overwrite").option("header", "true").csv(path))
...
def pivot_table(df):
    ...
    summary = df.groupBy("Contract", "Type").agg(
        F.sum(F.col("TotalDuration").cast("long")).alias("TotalDuration")
    )

    # 2) pivot để tạo cột cho mỗi Type
    pivoted = summary.groupBy("Contract").pivot("Type").sum("TotalDuration").na.fill(0)
    return pivoted
